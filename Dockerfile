...
RUN rmdir /q /s "%ProgramData%\Package Cache" && choco install --no-progress -y windows-sdk-10-version-2004-all || ( type %ProgramData%\chocolatey\logs\chocolatey.log && type %TMP%\chocolatey\*.log )
